...
def generate_random_probability(mu=0.8, sigma=0.5):
...
def generate_fake_preds(preds_path, labels, persist=False):
    ...
    for label in labels:
        ...
        for i in range(pred_num_for_label):
            x, y, r, sc = generate_fake_pred(label)
            if persist:
